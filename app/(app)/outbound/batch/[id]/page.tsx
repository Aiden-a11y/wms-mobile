...
function BatchPickInner() {
  ...
  async function loadAssignments(b: Batch) {
    ...
    try {
      const res = await fetch(`/api/wms/shipping/items/${encodeURIComponent(firstOrder.orderCode)}`, {
        headers: authHeaders(),
      });
      ...
      const infos: SkuInfo[] = b.skuList.map((skuEntry) => {
        const asgn = assignments.find((a) => a.productSku === skuEntry.sku) ?? null;
        return {
          sku: skuEntry.sku,
          name: skuEntry.name,
      ...
      // If we can't load assignments, still show SKUs without location
      setSkuInfos(b.skuList.map((s) => ({
        sku: s.sku, name: s.name,
        qtyPerOrder: s.qty, totalQty: b.orderCount * s.qty,
        assignment: null,
      })));
    }
  ...
  useEffect(() => {
    if (step === "location") setTimeout(() => inputRef.current?.focus(), 100);
  }, [step, currentIdx]);
  ...
  const allDone = doneSet.size >= skuInfos.length && skuInfos.length > 0;
            ...
            {skuInfos.map((info, i) => (
              <div key={info.sku} className="px-4 py-3 flex items-center gap-3"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {info.assignment && (
                    <p className="text-xs text-slate-500 truncate">{locLabel(info.assignment)}</p>
                  )}
                </div>
  ...
  async function handleLocationScan() {
    ...
    const expectedLabel = current.assignment ? locLabel(current.assignment) : "";

    // Try to match scanned value against location code or label
    const scanN = normalize(raw);
    const locN = normalize(expected);
    const labelN = normalize(expectedLabel);

    if (!expected || scanN === locN || locN.includes(scanN) || scanN.includes(locN) || scanN === labelN || labelN.includes(scanN)) {
      setLocScan(""); setLocLoading(false);
      markDone();
      return;
    }
    ...
    setLocError(expected ? `Wrong location. Expected: ${expectedLabel || expected}` : "Location not assigned — please assign from dashboard first");
    setLocLoading(false);
  }
  ...
  function markDone() {
    ...
    const nextIdx = skuInfos.findIndex((_, i) => i > currentIdx && !doneSet.has(i));
    if (nextIdx >= 0) {
      ...
      // Check if there are any undone before current
      const anyUndone = skuInfos.findIndex((_, i) => !doneSet.has(i) && i !== currentIdx);
      if (anyUndone >= 0) {
        setCurrentIdx(anyUndone);
        setStep("location");
        setLocScan("");
        setLocError("");
      }
      // else allDone will be true on next render
    }
  }
      ...
      <header className="px-5 py-4 flex items-center gap-3 flex-shrink-0" style={HDR_BORDER}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500">
            SKU {currentIdx + 1}/{skuInfos.length} · {batch.orderCount} orders
          </p>
        </div>
        <div className="flex items-center gap-1.5">
          {skuInfos.map((_, i) => (
            <div key={i} className="w-2 h-2 rounded-full transition-all"
              style={{ background: doneSet.has(i) ? "#22c55e" : i === currentIdx ? "#a78bfa" : "rgba(255,255,255,0.15)" }} />
          ))}
        </div>
      </header>
      ...
      <main className="flex-1 px-4 pt-4 pb-8 space-y-3 overflow-y-auto">
        {loadingAssign && (
          <div className="flex items-center gap-2 px-4 py-3 rounded-xl text-xs text-violet-300"
            ...
            Loading assigned locations from WMS…
          </div>
        )}

        {/* SKU progress overview */}
        <div className="rounded-2xl overflow-hidden" style={GLASS}>
            ...
            return (
              ...
              >
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                  isDone ? "bg-green-500/20" : isCurrent ? "bg-violet-500/20" : "bg-white/5"
                }`}>
                  {isDone
                ...
                <div className="flex-1 min-w-0">
                  <p className={`font-mono text-sm font-bold ${isDone ? "text-slate-500" : isCurrent ? "text-white" : "text-slate-400"}`}>
                    {info.sku}
                  </p>
                  {info.assignment
                ...
                <p className={`text-sm font-bold flex-shrink-0 ${isDone ? "text-green-400" : isCurrent ? "text-violet-300" : "text-slate-500"}`}>
                  ×{info.totalQty}
                </p>
              </button>
            );
        ...
        {current && !doneSet.has(currentIdx) && (
          <>
            ...
            <div className="rounded-2xl p-4" style={GLASS}>
              ...
              {current.assignment ? (
                <>
                  <p className="text-xl font-bold font-mono text-white">{locLabel(current.assignment)}</p>
                  <p className="text-xs text-slate-500 mt-0.5 font-mono">{current.assignment.location}</p>
                  {current.assignment.lotNo && (
                    <p className="text-xs text-slate-500 mt-1">LOT: {current.assignment.lotNo}
                      {current.assignment.expireDate ? ` · EXP: ${current.assignment.expireDate}` : ""}
                    </p>
                  )}
                </>
              ) : (
                <p className="text-sm text-amber-400">No location assigned yet — assign from dashboard</p>
              )}
            </div>

            {/* Qty info */}
            <div className="rounded-2xl p-4 flex items-center gap-4"
              style={{ ...GLASS, border: "1px solid rgba(139,92,246,0.3)", background: "rgba(139,92,246,0.08)" }}>
              <Package className="w-8 h-8 text-violet-400 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="font-mono text-base font-bold text-white">{current.sku}</p>
                {current.name && <p className="text-xs text-slate-400 mt-0.5 truncate">{current.name}</p>}
                <p className="text-xs text-slate-500 mt-1">{current.qtyPerOrder}/order × {batch.orderCount} orders</p>
              </div>
            ...
            {current.assignment && (
              <div className="rounded-2xl p-4 space-y-4"
                style={{ ...GLASS, border: "1px solid rgba(139,92,246,0.35)" }}>
                <div className="flex items-center gap-2">
                  <ScanLine className="w-4 h-4 text-violet-400" />
                  <p className="text-xs font-semibold text-violet-300 uppercase tracking-wider">Scan Location to Confirm</p>
                </div>
                {locError && (
